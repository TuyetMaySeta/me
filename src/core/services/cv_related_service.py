...
from src.repository.cv_related_repository import (
    ...
    SoftSkillRepository, ProjectRepository, CVRelatedBulkOperations
)
...
class CVRelatedService:
    ...
    def __init__(self, db_session: Session):
        ...
        self.cv_repository = CVRepository(db_session)
    ...
    def get_languages_by_cv(self, cv_id: str) -> List[LanguageResponse]:
        ...
        logger.info(f"Getting languages for CV: {cv_id}")
        
        self._check_cv_exists(cv_id)
    ...
    def get_technical_skills_by_cv(self, cv_id: str) -> List[TechnicalSkillResponse]:
        ...
        logger.info(f"Getting technical skills for CV: {cv_id}")
        
        self._check_cv_exists(cv_id)
        skills = self.tech_skill_repo.get_skills_by_cv_id(cv_id)
    ...
    def get_soft_skills_by_cv(self, cv_id: str) -> List[SoftSkillResponse]:
        ...
        logger.info(f"Getting soft skills for CV: {cv_id}")
        
        self._check_cv_exists(cv_id)
        skills = self.soft_skill_repo.get_soft_skills_by_cv_id(cv_id)
    ...
    def get_projects_by_cv(self, cv_id: str) -> List[ProjectResponse]:
        ...
        logger.info(f"Getting projects for CV: {cv_id}")
    ...
    def get_cv_components(self, cv_id: str) -> Dict[str, List]:
        ...
        logger.info(f"Getting all components for CV: {cv_id}")
    ...
    def bulk_create_components(self, request: BulkComponentCreateRequest) -> BulkComponentResponse:
        ...
        logger.info(f"Bulk creating components for CV: {request.cv_id}")
        ...
        try:
            ...
            if request.languages:
                # Remove cv_id from individual requests and use the main cv_id
                components_data['languages'] = [
                    {**lang.model_dump(exclude={'cv_id'}), 'proficiency': lang.proficiency.value}
                    for lang in request.languages
                ]
            
            if request.technical_skills:
                components_data['technical_skills'] = [
                    {**skill.model_dump(exclude={'cv_id'}), 'category': skill.category.value}
                    for skill in request.technical_skills
                ]
            
            if request.soft_skills:
                components_data['soft_skills'] = [
                    {**skill.model_dump(exclude={'cv_id'}), 'skill_name': skill.skill_name.value}
                    for skill in request.soft_skills
                ]
            
            if request.projects:
                components_data['projects'] = [
                    proj.model_dump(exclude={'cv_id'})
            ...
            logger.info(f"Bulk component creation completed for CV {request.cv_id}: {created_counts}")
            ...
            logger.error(f"Bulk component creation failed for CV {request.cv_id}: {str(e)}")
